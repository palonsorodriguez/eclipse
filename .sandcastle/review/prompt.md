...
1. Run `npm run typecheck` and `npm test` — confirm the current state passes.
...
3. Run `npm run typecheck` and `npm test` again. If either fails, fix it before continuing — do not leave the branch broken.
