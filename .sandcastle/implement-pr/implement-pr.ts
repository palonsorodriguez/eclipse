...
// The type argument is explicit: `Output.object` infers its payload from the
// schema's Standard Schema types, which resolve to `any` here, and an `any`
// output silently disables every check downstream of `result.output`.
const result = await runWithExtraction<z.infer<typeof ImplementPrOutput>>({
